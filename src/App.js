...
function App() {
  ...
  async function handleAdd() {
    ...
    await axios.post("https://todolist-latest-lbz4.onrender.com/api/todos", {
      title: title,
      completed: false,
    });
    setTitle("");
  ...
  async function handleEdit(item, value) {
    ...
    await axios.put(
      `https://todolist-latest-lbz4.onrender.com/api/todos/${item.id}`,
      {
  ...
  async function handleDelete(id) {
    await axios.delete(
      `https://todolist-latest-lbz4.onrender.com/api/todos/${id}`
    );
    getTasks();
  }
  ...
  async function getTasks() {
    try {
      const response = await axios.get(
        "https://todolist-latest-lbz4.onrender.com/api/todos"
      );
  ...
  return (
    <div className="App vh-100 p-0 ">
      <div className="d-flex flex-column align-items-center justify-content-center vh-100">
        <h1 className="mb-4" style={{ color: "#0ff" }}>
          To Do List
        </h1>
            ...
            <Row key={item.id}>
              <Col>
                <input
                  ...
                  onChange={() => handleEdit(item, item.completed)}
                />
                <input
                  type="text"
                  value={item.title}
                  ...
                  className={`my-2 task-input p-2 w-75 rounded m-3 ${
